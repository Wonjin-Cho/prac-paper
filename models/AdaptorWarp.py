...
class AdaptorWarp(nn.Module):
    def __init__(self, model) -> None:
        ...
        self.module2preconvs = {}
        self.name2preconvs = {}
        self.prehandles = {}
        self.module2afterconvs = {}
        self.name2afterconvs = {}
        self.afterhandles = {}
        self.module2name = {v: k for k, v in self.named_modules_dict.items()}


    def add_preconv_for_conv(self, name, convmodule=None, preconv=None):
        assert name in self.named_modules_dict, f"{name} not in model modules"
        ...
        if name in self.name2preconvs:
            print(f"Not inserting because {name} already has a pre-conv")
            return None
        if preconv is None:
        ...
        self.add_module(f"preconv_{len(self.name2preconvs)}", preconv)  # Register preconv as a submodule
        ...
        # print(f"=> Added pre-conv on {name}")
        return preconv
    ...
    def add_afterconv_for_conv(self, name, convmodule=None, afterconv=None):
        ...
        if name in self.name2afterconvs:
            print(f"Not inserting because {name} already has an after-conv")
            return None
        ...
        self.add_module(f"afterconv_{len(self.name2afterconvs)}", afterconv)  # Register afterconv as a submodule
        ...
        # print(f"=> Added after-conv on {name}")
        return afterconv
    ...
    def remove_preconv_for_conv(self, name, convmodule=None, absorb=False):
        ...
        if name not in self.name2preconvs:
            if absorb:
                print(f"WARN: Cannot absorb because no pre-conv found for {name}")
            return
        if absorb:
            print(f"=> Absorbing pre-conv on {name}")
        ...
        # print(f"=> Removed pre-conv on {name}")
    ...
    def remove_afterconv_for_conv(self, name, convmodule=None, absorb=False):
        ...
        if name not in self.name2afterconvs:
            if absorb:
                print(f"WARN: Cannot absorb because no after-conv found for {name}")
            return
        if absorb:
            print(f"=> Absorbing after-conv on {name}")
        ...
        # print(f"=> Removed after-conv on {name}")

    def forward(self, x: Tensor) -> Tensor:
        self.model.get_feat = self.get_feat
        return self.model(x)

    def remove_all_preconv(self, absorb=False):
        """
        Removes all pre-conv layers from the model. Optionally absorbs the pre-conv into the corresponding conv layer.
        """
        conv_names = list(self.name2preconvs.keys())  # Collect all keys to avoid modifying dict during iteration
        for name in conv_names:
            self.remove_preconv_for_conv(name, absorb=absorb)
        # print("=> Removed all pre-convs from the model")

    def remove_all_afterconv(self, absorb=False):
        """
        Removes all after-conv layers from the model. Optionally absorbs the after-conv into the corresponding conv layer.
        """
        conv_names = list(self.name2afterconvs.keys())  # Collect all keys to avoid modifying dict during iteration
        for name in conv_names:
            self.remove_afterconv_for_conv(name, absorb=absorb)
        # print("=> Removed all after-convs from the model")
...
#         self.module2preconvs[convmodule] = preconv
#         self.name2preconvs[name] = preconv
#         # self.named_modules_dict[name] = preconv
#         def hook(module, input):
#             preconv = self.module2preconvs[module]
#             return preconv(input[0])
#         self.prehandles[convmodule] = convmodule.register_forward_pre_hook(hook)
...
#             afterconv.weight.data.copy_(torch.eye(v).view(v, v, 1, 1))
#         self.module2afterconvs[convmodule] = afterconv
#         print("value of afterconv and module")
#         print(afterconv)
#         print(module)
#         self.name2afterconvs[name] = afterconv
#         def are_convs_equal(conv1, conv2):
#             return (
#             conv1.in_channels == conv2.in_channels and
...
#             conv1.bias is not None == conv2.bias is not None
#             )
#         def hook(module, input, output):
#             # print(module)
#             # print(convmodule)
#             #print(self.module2afterconvs)
#             #afterconv = module
#             # for key in self.module2afterconvs:
#             #     if are_convs_equal(module, key):  # Check structural equality
#             #         print("value of modules and keys")
#             #         print(module)
#             #         print(key)
#             v = convmodule.out_channels
#             # afterconv = nn.Conv2d(v, v, kernel_size=1, stride=1, padding=0, bias=False).cuda()
...
#     #     def hook(module, input, output):
#     #         # Use self.module2name to retrieve the module name
#     #         module_name = self.module2name.get(module, "Unknown")
#     #         # print(f"Hook triggered for module: {module_name}")
#     #         afterconv = self.name2afterconvs[module_name]
#     #         return afterconv(output)
